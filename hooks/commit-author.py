...
numeric id prefix (12345+name@...), which GitHub does not link to anyone.
  ...
  LEAN_ALLOW_COMMIT_AUTHOR=1   turn it off
...
def email_problem(email, hosts=None):
    ...
    labels = domain.split(".")
    tld = labels[-1]
    if len(labels) < 2 or not re.fullmatch(r"[a-z]{2,63}", tld) or tld in LOCAL_SUFFIXES:
...
def problems(command, cwd, hosts=None):
    ...
    for seg in hookkit.segments(command, cwd):
        ...
        if not call or call[0] != "commit" or "--dry-run" in call[1]:
            continue
        problem = email_problem(effective_email(seg, call), hosts)
        if problem:
...
def main():
    ...
    if found:
        print("Blocked by lean-agent, the commit author email is wrong: " + "; ".join(found)
              ...
              "and run the command again. LEAN_ALLOW_COMMIT_AUTHOR=1 turns this check off.",
              file=sys.stderr)
        return 2
    return 0
...
def self_test():
    ...
    bad = ["", "opc@devbox", "me@devbox.localdomain", "me@box.lan", "me@devbox.example.org",
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for command, want in cases.items():
                if bool(problems(command, repo, hosts)) != want:
                    ...
                    failures += 1
            subprocess.run(["git", "-C", repo, "config", "user.email", "opc@devbox"], check=True)
