...
- Hooks, each with its own off switch. Two block, the rest add a note for the agent and never stop the call:
  ...
  - `record-test-run.py` and `test-before-push.py` record each passing compiled test run and note, on `git push`, `gh pr create` or `gh pr merge`, the changed source files edited since. Off: `LEAN_SKIP_TEST_BEFORE_PUSH=1`.
  ...
  - `lesson-log.py` appends a commit whose subject says fix, revert, flaky, silent, gate or regress to `.lean/lessons.tsv` (format in the script header) for the retro. Off: `LEAN_SKIP_LESSON_LOG=1`.
